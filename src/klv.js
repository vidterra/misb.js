...
module.exports.calculateChecksum = (packet) => {
	packet = typeof packet === 'string' ? Buffer.from(packet, 'hex') : packet
	let total = 0
...
module.exports.is0806ChecksumValid = (buf, checksum) => {
	...
	//console.log((crc & 0xffffffff), checksum, (crc & 0xffffffff) === checksum)
...
module.exports.parseStandard = (standard, buffer, options = {}) => {
	...
	let values = null
	try {
		values = standard.parse(buffer.subarray(0, standard.key.length + berHeader + berLength + contentLength), options)
	} catch (e) {
		console.debug(e)
	}
		...
		try {
			for (const standard of standards) {
				if (module.exports.startsWithKey(buffer, standard.key)) {
					const {index, values} = module.exports.parseStandard(standard, buffer, options)
					if (values) {
