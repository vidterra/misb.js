...
process.stdin.on('data', function (data) {
	const result = klv.decode(data, standards, null, {verbose: true, payload: true, debug: process.argv[2] === 'debug' })
	for (const standard of standards) { // only decode 0601
		for(const packet of result[standard.name]) { // may have multiple 0601 packets from standard in
			...
			st0601.parse(result, {verbose: true, payload: true, debug: process.argv[2] === 'debug' })
			process.exit(0)
		}
	}
})
